...
While such models are very successful in reproducing a wide variety of properties of aqueous systems~\cite{RZK-Vega2011} [it has been argued that] the failure to include covalent effects properly limits their applicability to RZK~\cite{RZK0-weinhold}. % RZK: only if covalent interactions are substantially different from fixed electrostatics
...
Even the small amount of electron density donated from a neighbor oxygen into the antibonding orbitals localized of an intramolecular O--H bond (see Figure ZZZ in Ref.~\onlinecite{khaliullin2009dimer}) is sufficient to soften the O--H bond and thus lower its vibrational frequency by ZZZ~cm$^{-1}$. 
...
%RZK: The x- and y-axis labels are confused. 
\end{figure}
