...
The 0.7~ps HB lifetimes calculated from the time-correlation functions shown in Figure~\ref{Fig:HBdecay}SZZZ agree well with experimental estimates \cite{lawrence2003ultrafast} and simulations~\cite{marti1996molecular,starr1999fast}. Removing intermolecular covalency shotens the HB lifetime by an order of magnitude to 0.08~ps. 
...
\textbf{Removal of intermolecular covalency.} Quick review of ALMO and dual-basis feature of CP2K.

\textbf{Ab initio molecular dynamics without intermolecular covalency.} Implementation of energies and FORCES in CP2K based on HF theorem.
...
%RZZ: make sure that all references are formatted correctly. It is very likely that the formatting will change if the manuscript is not accepted by our top-choice journal. It is, therefore, better to make sure everything can be reformatted with one click.
