...
Many remarkable properties of liquid water originate from the ability of water molecules to form hydrogen bonds, each of which is a combination of electrostatic, polarization, dispersion, and covalent interactions. 
In this work, an \emph{ab initio} molecular dynamics method with restricted electrons is utilized to switch off the covalent component of interactions between water molecules in simulations. 
Comparison of simulations with restricted and unrestricted electrons shows that a small amount of intermolecular electron transfer between two molecules has a profound effect on the structure and dynamics of the hydrogen-bond network and observable properties of room-temperature liquid water. 
The findings of this study deepen our knowledge about the nature of hydrogen bonding and will facilitate the development of more reliable water models and aid interpretation of solvation and catalytic effects in aqueous systems.
\end{abstract}
...
The 14\% decrease in the density should correspond to a 5\% elongation of intermolecular distances assuming that they all increase uniformly. Nevertheless, the position of peaks in RDF remains the same in the devalent water at two different densities (Figure~\ref{Fig:rdf_cp} in Supplementary Information). The only noticeable structural change is that peaks become slightly broader in the low-density constant-pressure model and angular distribution is more narrow (Figure~\ref{Fig:adfcp}). This indicates that most molecules stay in the same position around the minimum of the potential well when the density is allowed to relax. Only a small fraction of them becomes distorted in the radial and angular directions. 
...
Apparently even small amount of electron density transferred into the antibonding orbitals of the intramolecular O--H bond (see Figure~2 in Ref.~\onlinecite{khaliullin2009electron}) is sufficient to ``soften'' the O--H bond and thus lower its vibrational frequency by 600~cm$^{-1}$.
...
Finally, a comparison of the stretching bands for the three models shows that the spectacular enhancement of the integrated intensity from the gas to liquid phase is not entirely due to the intermolecular electron transfer as has been usually assumed~(see Chapter 5 in Ref.~\onlinecite{HBbook}). It is known that the integrated intensity is proportional to $\left({\partial \mu}/{\partial q}\right)^2$ -- derivative of the dipole moment with respect to the coordinate of the vibrational mode (i.e. intramolecular O--H distance) -- and the change in the integrated intensity arises mostly from the change in this derivative. The change in the derivative is attributed to decoupling of the motion electrons from that of the nuclei in HB systems: stretching of a HB-bonded O--H group causes the electrons (i.e. Wannier centers in Figure~\ref{Fig:acoord}) not to follow the H-atom as much as when O--H does not establish a HB~\cite{HBbook}. Our data shows that the derivative changes both from the gas-phase to devalent model and from the devalent to realistic model. This means that the mere presence of neighbor lone pairs leads to decoupling while the intermolecular electron transfer makes it more pronounced.
...
%RZK0: Origin of the increased intensity? 
...
%RZK0: make sure that all references are formatted correctly. It is very likely that the formatting will change if the manuscript is not accepted by our top-choice journal. It is, therefore, better to make sure everything can be reformatted with one click.
\bibliography{covalency}

%RZK0: uncomment the switch to separate SI from the main text
%\else % maintext - SI switch

%RZK0: references to all sections from the main text and Computational Methods
...
% RZK: change two axis labels from "OO" to "OH" in the second panel. Change legend to "Fixed-density devalent" and "Relaxed-density devalent"
\end{figure} 
