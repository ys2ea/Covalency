...
The concept of intermolecular covalency, described canonically as lone electron pairs of oxygen atoms donated to nearby hydrogens, has been tremendously useful to describe physics and chemistry of water.
%[and is deeply embedded into the everyday chemistry jargon/language.] 
...
Recent developments in energy decomposition techniques based on accurate electronic structure methods have helped make substantial progress towards quantifying the individual contributions of various physical effects on the HB energy in small water clusters~\cite{ZZZ}. 
...
A deep connection between the covalent interactions and features of the X-ray absorption~\cite{NatureComm2013,ZZZ}, infrared~\cite{JPCL2013,Faraday2011}, and nuclear magnetic resonance~\cite{NatureComm2015,ZZZ} spectra of liquid water and small water clusters has been pointed out in several recent articles. 
...
%Faraday2011: E. Ramos-Cordoba, D. S. Lambrecht, M. Head-Gordon, Charge-transfer and the hydrogen bond: Spectroscopic and structural implications from electronic structure calculations, Faraday Discuss. 150, 345, 2011.
% ZZZ We need more citations! "charge transfer" "IR shift" "XAS is different for distorted bonds bonds"
...
Unfortunately, modern exchange-correlation functionals cannot describe the energetics of intermolecular interactions precisely and, therefore, can reproduce properties of liquid water only in \emph{semi-quantitative} agreement with experimental measurements. 
...
Like most generalized gradient approximation (GGA) and meta-GGA functionals, BLYP is known to underestimate the energy gap of water molecules~\cite{PNAS2017, JPCL2012}.
% ZZZ - add the following
% PNAS, 2017, 114 (41), 10846
% J. Phys. Chem. Lett., 2012, 3 (23), 3411.
Because of this, the strength of intermolecular binding is overestimated and computed radial (angular) distribution functions have  sharper peaks than those derived from X-ray (NMR) scattering experiments~\cite{ZZZ}. 
The overbinding effect also results in a somewhat underestimated diffusion constant~\cite{ZZZ}, overestimated viscosity~\cite{ZZZ}, and red-shifted peaks in the high-frequency region of the IR spectrum~\cite{ZZZ}. 
...
%RZK: it is extremely important not to give quantitative extimates unless required.
...
For comparison, the HBs become truly non-directional with uniform angular distribution in a water model represented only by the Lennard-Jones potential (Figure~\ref{Fig:SDF}C). %retains only dispersion interactions and short-distance repulsion,  
...
However, several interesting qualitative conclusions can be drawn from the models employed in this work, without resorting to high-level calculations. 
...
Apparently, even the small amount of electron density transferred to the antibonding orbitals of intramolecular O--H bonds (see Figure~2 in Ref.~\citenum{khaliullin2009electron}) is sufficient to ``soften'' the O--H bond---that is, decrease its force constant---and lower its vibrational frequency. 
...
Finally, a comparison of the stretching bands for the three models shows that the spectacular enhancement of the integrated intensity from the gas to the liquid phase is not entirely due to the intermolecular electron transfer as was previously assumed~(see Chapter 5 in Ref.~\citenum{marechal2006hydrogen}). 
...
% RZK: Remind that the dipole is a result of all three types of interactions.
...
It is important to clarify that all types of interactions---frozen density, polarization and covalent--- contribute to the distribution of electrons around molecules and, therefore, the magnitude of molecular dipoles. 
...
At the same time, the wider spread of dipole moments in the reference model (Figure~\ref{Fig:dipoledist}) is attributable to the four-fold increase in the spread of positions of electron lone pair centers. 
To be precise, intermolecular charge transfer increases the standard deviation from the average position of the lone pair centers from 0.07~pm in the devalent model to 0.29~pm in the reference model.
%
...
% RZK: Converging the Dielectric constant to get precise might require a significantly longer tranjectory. Our trajectory is sufficient to see drastic qualitative difference between the two models.
...
%RZK RW3  I do not agree with the argument presented by the authors regarding the convergence of the dielectric constant. The fact that previous, and relatively old, studies used very short trajectories to calculate the dielectric constant, without actually demonstrating any convergence, seems not to represent a particularly strong justification. The authors may want to consider the discussion presented in Phys. Rev. B 93, 144201 (2016) and references 3-26 therein. In particular, Ref. 25 of this PRB paper (J. Chem. Phys. 140, 124504 (2014)) clearly shows that several nanoseconds are needed to converge the dielectric constant. This implies that both values and error bars reported by the authors are, at least,  questionable.
...
While the dielectric constant for the reference model is 70, in good agreement with the experimental value of 78, this value increases to a remarkable 180 for the devalent model.
...
In addition to providing an estimate of the contribution of the covalent component on the properties of water, our work reveals that intermolecular covalency is entirely responsible for the large redshift and broadening of the O-H stretching peaks in its IR spectra. It is only partially responsible for a dramatic increase in the intensity of these peaks. 
