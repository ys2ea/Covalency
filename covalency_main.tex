...
% Accurate quantitative estimates of the in this work
...
%lone electron pairs of oxygen atoms donated to hydrogens is a canonical model of intermolecular covalency
...
The 14\% decrease in density should correspond to a 5\% elongation in intermolecular distances, assuming a uniform increase. Nevertheless, the position of peaks in the RDF remains unchanged in the devalent water at the two different densities (Figure~\ref{ext1-Fig:rdf_cp} in Supplementary Information). The only noticeable structural changes are that peaks in the RDF become slightly broader in the low-density constant-pressure model (Figure~\ref{ext1-Fig:rdf_cp}), and the angular distribution is more narrow (Figure~\ref{ext1-Fig:ir_cp}). This indicates that most molecules stay in the same position around the minimum of the potential well when the density is allowed to relax. Only a small fraction of them become distorted radially or angularly. 
...
Apparently, even the small amount of electron density transferred to the antibonding orbitals of intramolecular O--H bonds (see Figure~2 in Ref.~\citenum{khaliullin2009electron}) is sufficient to ``soften'' the O--H bond---that is, decrease its force constant--- and lower its vibrational frequency by 600~cm$^{-1}$.
